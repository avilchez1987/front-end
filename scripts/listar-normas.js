...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.loadSectores = async function(page = 1) {
        try {
            ...
            const response = await fetch(`${baseUrl}/api/normas/listarNormas?limite=${pageSize}&desde=${desde}`, {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                normas.forEach(norma => {
                    ...
                    const estado = norma.estado ? 'Activo' : 'Inactivo';
                    ...
                    row.innerHTML = `
                        ...
                        <td>${norma.fechaCreacion}</td>
                        <td>${norma.usuarioCreacion.email}</td>
                        <td>${estado}</td>
                        <td>
                        <button class="btn btn-edit" onclick="editUser('${norma.uid}')"><i class="fas fa-edit"></i> Editar</button>
                        <button class="btn btn-delete" onclick="deleteSector('${norma._id}')"><i class="fas fa-trash-alt"></i> Eliminar</button>
                        </td>
                    `;
...
window.deleteSector = async function(_id) {
    ...
    if (confirm('¿Estás seguro de que deseas eliminar esta norma?')) {
        try {

            const response = await fetch(`${baseUrl}/api/normas/borrarNorma/${_id}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'x-token': token
